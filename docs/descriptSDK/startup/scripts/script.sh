...
setGlobals () {
    ...
    export CORE_PEER_ADDRESS CORE_PEER_TLS_ROOTCERT_FILE CORE_PEER_MSPCONFIGPATH
    if [[ $1 == 0 || $1 -eq 1 ]] ; then
...
joinWithRetry () {
    ...
    res=$?
    cat log.txt
    if [[ $res != 0 && $COUNTER < $MAX_RETRY ]]; then
...
instantiateChaincode () {
    ...
    cat log.txt
    verifyResult $res \
        "Chaincode instantiation on PEER$PEER on channel '$CHANNEL_NAME' failed"
    # Ignore LineLengthBear
...
chaincodeInvoke () {
    ...
    if [[ -z "$CORE_PEER_TLS_ENABLED" || "$CORE_PEER_TLS_ENABLED" == "false" ]]; then
        peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME \
             -n mycc -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    else
